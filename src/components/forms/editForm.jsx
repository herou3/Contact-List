...
const EditForm = ({ data, validatorConfig, onChange, isHaveDate, onCompleteChange, onChangeMode }) => {
      ...
      <div className="row">
        <div className="row justify-content-md-center">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <TextField labelText="Birthday" type="date" id="birthday" name="birthday" value={data.birthday} onChange={onChange} error={errors.birthday}></TextField>
            <TextField labelText="Url" id="url" name="url" value={data.url} onChange={onChange} error={errors.url}></TextField>
            <div className="d-grid gap-2 col-6 mx-auto">
              <button disabled={!isValid} className="btn btn-primary">{isHaveDate ? "Update" : "Create"}</button>
              {isHaveDate && (
                <button className="btn btn-secondary" onClick={onChangeMode}>Back</button>
              )}
            </div>
          </form>
        </div>
      </div>
      {isHaveDate &&  <InfoModel title="Success" text={isCreated ? "User " + fullName + " was updated" : "User was created"} isOpen={isOpenPopup} handleClose={handleClose} />}
    </div>
  )
}
